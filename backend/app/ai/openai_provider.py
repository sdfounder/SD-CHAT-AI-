...
class OpenAIProvider(BaseLLMProvider):
    """
    Fournisseur officiel OpenAI (API Directe v1).
    Supporte les modèles gpt-4o, gpt-4o-mini, o3-mini avec streaming SSE standard.
    """
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            async with client.stream("POST", url, headers=headers, json=payload) as response:
                if response.status_code != 200:
                    ...
                    raise RuntimeError(f"Erreur OpenAI {response.status_code}: {err_text[:200]}")
    ...
    async def send_message(
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(30.0, connect=8.0)) as client:
            ...
            if response.status_code != 200:
                ...
                raise RuntimeError(f"Erreur OpenAI {response.status_code}: {err_text}")
    ...
    def get_provider_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model": self._model,
            "masked_key": f"sk-...{self._api_key[-4:]}" if self._api_key and len(self._api_key) > 8 else None,
        }
    ...
    def classify_error(self, error: Exception) -> str:
        ...
        if any(c in err_str for c in ("500", "502", "503", "504", "bad gateway")):
            return "service_unavailable"
        if "timeout" in err_str or "timed out" in err_str:
            return "timeout"
        if "401" in err_str or "invalid_api_key" in err_str:
            return "auth_error"
        if "moderation" in err_str or "content policy" in err_str:
    ...
    def estimate_usage(self, prompt: str, completion: str) -> Dict[str, Any]:
        ...
        in_cost = info.get("cost_input_per_million", 0.15)
        out_cost = info.get("cost_output_per_million", 0.60)
