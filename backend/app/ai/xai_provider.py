...
class XAIProvider(BaseLLMProvider):
    """
    Fournisseur officiel xAI (Grok API v1).
    Supporte grok-2-1212 et grok-2-vision-1212 avec streaming SSE.
    """
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            async with client.stream("POST", url, headers=headers, json=payload) as response:
                if response.status_code != 200:
                    ...
                    raise RuntimeError(f"Erreur xAI {response.status_code}: {err_text[:200]}")
    ...
    async def send_message(
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(30.0, connect=8.0)) as client:
            ...
            if response.status_code != 200:
                ...
                raise RuntimeError(f"Erreur xAI {response.status_code}: {err_text}")
    ...
    def get_provider_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model": self._model,
            "masked_key": f"xai-...{self._api_key[-4:]}" if self._api_key and len(self._api_key) > 8 else None,
        }
